<script>
  ...
  import { getHouseholdWeight, selectWeightedHouseholds } from '../data/householdWeight.js';
  ...
  // Safari needs a point cap; cache a deterministic, weight-aware sample so
  // the same households remain visible throughout animation frames.
  let renderSampleSource = null;
  let renderSamplePinnedKey = '';
  let renderSample = [];
  ...
  function getDataForRendering() {
    ...
    if (data.length <= maxPointsToRender) return data;

    const pinnedIds = [
      selectedHousehold?.id,
      ...Object.values(randomHouseholds).map((household) => household?.id)
    ].filter(Boolean);
    const pinnedKey = pinnedIds.map(String).sort().join('|');

    if (renderSampleSource !== data || renderSamplePinnedKey !== pinnedKey) {
      renderSampleSource = data;
      renderSamplePinnedKey = pinnedKey;
      renderSample = selectWeightedHouseholds(data, maxPointsToRender, {
        pinnedIds
      });
    }
  ...
  function initializePointAnimations(dataPoints) {
    ...
    shuffledPoints.forEach((point, index) => {
      ...
      const finalDelay = Math.max(
        0,
        Math.min(scheduledDelay + randomVariation, actualAnimationDuration - 400)
      );
    ...
    console.log(
      `🌟 Starfield animation initialized: ${dataPoints.length} stars over ${(actualAnimationDuration / 1000).toFixed(1)}s`
    );
  ...
  function checkForDataChange() {
    ...
    if (data.length !== lastDatasetLength && data.length > 0) {
      console.log(
        `Data changed: ${lastDatasetLength} → ${data.length} points, starting starfield animation ✨`
      );
      ...
      if (isInitialLoad) {
        ...
        const shuffledIndices = Array.from({ length: totalPoints }, (_, i) => i).sort(
          () => Math.random() - 0.5
        );
  ...
  function renderCanvas() {
    ...
    try {
      ...
      let allRelevantData = dataForRendering;
      if (isTransitioning && fromState && toState) {
        allRelevantData = dataForRendering.filter((d) => {
        ...
        allRelevantData = dataForRendering.filter((d) =>
          currentView.filter ? currentView.filter(d) : true
        );
      }
  ...
  function renderPoints(ctx, data, xScale, yScale, weightRange, currentState, fromState) {
    ...
    data.forEach((d) => {
      ...
      const isGroupHighlighted =
        d.isHighlighted && d.stateIndex === Math.floor(currentStateIndex / 2);
      ...
      let radius = isHighlighted
        ? isRandomHighlighted || isSelectedHousehold
          ? 5
          : 3.5
        : baseRadius; // Increased highlighted sizes
      ...
      const normalizedWeight = logWeightRange > 0 ? (logWeight - logMinWeight) / logWeightRange : 1;
      ...
      let baseOpacity = isHighlighted
        ? 1
        : Math.min(Math.max(weightBasedOpacity, minOpacity), maxOpacity);
  ...
  function drawAxes(xScale, yScale) {
    ...
    yAxis
      .selectAll('.tick line')
      .style('stroke', COLORS.GRID_LINES)
    ...
    g.append('text')
      ...
      .text(isMobile ? 'Resource change (%) →' : 'Change in household resources (2026, %) →');
...
<div class="chart-container">
  ...
  ></canvas>
  <svg bind:this={svgRef} class="overlay-svg"></svg>
</div>
