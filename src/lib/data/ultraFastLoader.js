...
function processData(rawData) {
  return rawData.map((d, i) => ({
    ...
    'Total change in net income':
      d['Total change in net income'] || d['Change in Household Net Income'] || 0,
...
export async function loadDatasetUltraFast(selectedDataset, onUpdate) {
  ...
  try {
    ...
    console.log(
      `✅ Micro sample ready in ${phase1Time.toFixed(0)}ms - ${processedMicro.length} dots visible!`
    );
    ...
    console.log(
      `✅ Small sample ready in ${phase2Time.toFixed(0)}ms - ${processedSmall.length} dots`
    );
    ...
    console.log(
      `✅ Medium sample ready in ${phase3Time.toFixed(0)}ms - ${processedMedium.length} dots`
    );
    ...
    const [otherLargeData] = await Promise.all([loadSample(otherDataset, 'large')]);
